...
namespace WindowsFormsApp1
{
    public partial class confirm : Form
    {
        ...
        private void confirm_Load(object sender, EventArgs e)//เมื่อหน้ายืนยันโหลดขึ้นมา ให้ทำดังต่อไปนี้
        {
            //ดึงข้อมูลเลขที่ใบเสร็จจากสถานะผู้ใช้ ว่าค้างบิลอะไรบ้าง
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    no_receiptformDB_toProgram = reader.GetString("status_login");//ดึงค่า status_login เก็บไว้ในข้างซ้าย
                }
            }
            catch
            {
            ...
            try
            {
                ...
                con1.Close();
                payMoney1.Text = price_menu;//ดึงข้อมูลว่าต้องจ่ายเท่าไหร่ ในปุ่มยอดชำระ
        ...
        {
            try //ลบสถานะผู้ใช้ (บิลที่ค้าง) เป็นการลบเลขที่ใบเสร็จออก
            {
                MySqlConnection con1 = new MySqlConnection("host=localhost;user=admin;password=123456;database=project");
                con1.Open();
            ...
            try //ลบใบเสร็จทิ้ง 
            {
                ...
                cmd2.ExecuteReader();
                con2.Close();
                MessageBox.Show("ยกเลิกคำสั่งซื้อเรียบร้อยแล้ว", "การแจ้งเตือน");
                this.Close();
            }
            catch (Exception ex)
            {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                Paymoney = int.Parse(payMoney1.Text);
                Givemoney = Allmoney - Paymoney;
                giveMoneyTextbox.Text = Givemoney.ToString();


                payMoneyButton.Enabled = false;//เป็นคำสั่งปิดปุ่มเมื่อจ่ายเรียบร้อยแล้ว
                payMoneyButton.Text = "จ่ายแล้วเรียบร้อย";

                try //ลบสถานะผู้ใช้ (บิลที่ค้าง) ในกรณีที่จ่ายเรียบร้อยเเล้ว
                {
                    MySqlConnection con1 = new MySqlConnection("host=localhost;user=admin;password=123456;database=project");
                    con1.Open();
                    string sql1 = "UPDATE login SET status_login = '' WHERE Username = '" + Program.username + "';";
                    ...
                    Print.Enabled = true;
                    CancelButton.Enabled = false;//ปิดปุ่มยกเลิกไว้
                }
                catch (Exception ex)
                {
                    MessageBox.Show("ลบสถานะผู้ใช้ (บิลที่ค้าง)" + ex, "การแจ้งเตือน");
        ...
        private void Print_Click(object sender, EventArgs e)//ปริ้นใบเสร็จ
        {
